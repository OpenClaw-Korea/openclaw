...
이 설정은 종종 인터넷 전체 SSH 무차별 대입 공격을 막기 위한 추가 호스트 기반 방화벽 규칙의 *필요성*을 제거하지만, 여전히 OS를 업데이트된 상태로 유지하고 `openclaw security audit`를 실행하며 실수로 공개 인터페이스에서 수신하지 않는지 확인해야 합니다.
